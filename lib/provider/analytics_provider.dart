...
//
// All providers required by:
//   • analytics_screen.dart  – AnalyticsScreen
//   • family_screen.dart     – FamilyScreen
//   • profile_screen.dart    – ProfileScreen
//
// Providers declared here:
//
//   Analytics
//     analyticsRangeProvider        – StateProvider<String> ('week' | 'month')
//     stressAnalyticsProvider       – StreamProvider<StressAnalyticsReport>
//     moodCalendarProvider          – StreamProvider<Map<int, int>>
//     sleepQualityHistoryProvider   – StreamProvider<List<SleepEntry>>
//     topTriggersProvider           – (re-exported alias to stress_provider)
//
//   Family
//     familyRepositoryProvider      – Provider<FamilyRepository>
//     familyMembersProvider         – StreamProvider<List<FamilyModel>>
//     familyControllerProvider      – StateNotifierProvider<FamilyController>
//
//   Profile
//     notificationSettingsProvider  – StateNotifierProvider<NotifSettingsNotifier>
//     profileEditProvider           – StateNotifierProvider<ProfileEditNotifier>
//
// Value objects:
//     StressAnalyticsReport
//     SleepEntry
...
/// Fully computed analytics report consumed by _ReportBody.
class StressAnalyticsReport {
  ...
  final String bestDay;   // formatted date string
  final String worstDay;  // formatted date string
...
/// Single sleep-quality data point for the sparkline chart.
class SleepEntry {
  final DateTime date;
  final int value; // 1–5
...
/// 'week' | 'month' — drives which window the analytics screen shows.
...
/// Derives a full [StressAnalyticsReport] from the stress history stream.
/// Reacts to [analyticsRangeProvider] — switching week/month reruns this.
final stressAnalyticsProvider =
StreamProvider<StressAnalyticsReport>((ref) {
...
StressAnalyticsReport _buildReport(List<StressLogModel> logs) {
  ...
  // One entry per calendar day (keep last log per day)
...
/// Returns a Map<day-of-month, overallMood (1–5)> for the current month.
/// Consumed by _MoodCalendar in analytics_screen.dart.
final moodCalendarProvider = StreamProvider<Map<int, int>>((ref) {
  ...
  return _db
      ...
      .where('date',
      isLessThanOrEqualTo: endMonth.millisecondsSinceEpoch)
    ...
    for (final doc in snap.docs) {
      ...
      // putIfAbsent keeps the latest entry per day
...
/// Last 14 days of morning check-in sleep quality scores.
/// Consumed by _SleepQualityChart in analytics_screen.dart.
final sleepQualityHistoryProvider =
StreamProvider<List<SleepEntry>>((ref) {
  final uid = _uid();
  if (uid == null) return Stream.value([]);
...
// ─────────────────────────────────────────────────────────────────────────────
// topTriggersProvider alias
// ─────────────────────────────────────────────────────────────────────────────
// analytics_screen.dart calls ref.watch(topTriggersProvider) without a days
// argument (it expects a simple non-family provider).
// We expose a fixed-30-day convenience alias here.

/// Top stress triggers over the last 30 days.
/// Simple (non-family) alias consumed by _TopTriggers widget.
/// Returns List<MapEntry<String, int>> sorted by frequency desc.
...
StreamProvider<List<MapEntry<String, int>>>((ref) {
  ...
  return _db
    ...
    for (final doc in snap.docs) {
      final triggers =
      List<String>.from(doc.data()['triggers'] ?? []);
...
class FamilyRepository {
  ...
  /// Real-time stream of all family members for the current user.
  Stream<List<FamilyModel>> members(String uid) {
    return _col
        ...
        .snapshots()
        .map((s) => s.docs
        .map((d) => FamilyModel.fromMap(d.data()))
        .toList());
  }

  /// Adds a new pending family member invitation.
  Future<void> invite({
    ...
    final doc = FamilyModel(
      id:           id,
      ownerId:      ownerId,
      memberId:     '',          // empty until they accept
  ...
  /// Updates alert preferences for an existing member.
...
class FamilyController
    extends StateNotifier<FamilyControllerState> {
  final FamilyRepository _repo;

  FamilyController(this._repo)
      : super(const FamilyControllerState());
...
final familyControllerProvider = StateNotifierProvider<
    FamilyController, FamilyControllerState>(
...
class NotificationSettings {
  ...
  final int  morningHour;       // 24-h, default 8
  final bool eveningReminder;
  final int  eveningHour;       // 24-h, default 21
...
class ProfileEditNotifier
    extends StateNotifier<ProfileEditState> {
  final AuthRepository _authRepo;

  ProfileEditNotifier(this._authRepo)
...
// ─────────────────────────────────────────────────────────────────────────────
// Tier label extension — used in profile_screen.dart
// ─────────────────────────────────────────────────────────────────────────────
